...
import { loginRequest, graphConfig } from "@/lib/msalInstance"
...
export class SharePointService {
  ...
  private static async getAccessToken(): Promise<string> {
    ...
    if (accounts.length === 0) {
      throw new Error("No active MSAL account found. Please log in.")
    }
  ...
  /**
   * Fetches information about a SharePoint site.
   * @param siteUrl The full URL of the SharePoint site (e.g., https://yourtenant.sharepoint.com/sites/MySite).
   * @returns A promise that resolves with the site information.
   */
  static async getSiteInfo(siteUrl: string): Promise<any> {
    const accessToken = await this.getAccessToken()
    const encodedSiteUrl = encodeURIComponent(siteUrl)
    const url = `${graphConfig.graphSitesEndpoint}/${encodedSiteUrl}`
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to get site info: ${response.status} ${response.statusText} - ${errorData.error?.message || "Unknown error"}`)
    }
  ...
  /**
   * Fetches the contents of a specific folder within a SharePoint site.
   * @param siteUrl The full URL of the SharePoint site.
   * @param folderPath The relative path to the folder (e.g., "Shared Documents/MyFolder").
   * @returns A promise that resolves with an array of folder items (files and subfolders).
   */
  static async getFolderContents(siteUrl: string, folderPath: string): Promise<any[]> {
    const accessToken = await this.getAccessToken()
    const encodedSiteUrl = encodeURIComponent(siteUrl)
    const encodedFolderPath = encodeURIComponent(folderPath)

    // Graph API path for folder contents: /sites/{site-id}/drive/root:/{path}:/children
    // First, get site ID
    const siteInfo = await this.getSiteInfo(siteUrl)
    const siteId = siteInfo.id

    if (!siteId) {
      throw new Error("Could not retrieve site ID from the provided URL.")
    }

    const url = `${graphConfig.graphSitesEndpoint}/${siteId}/drive/root:/${encodedFolderPath}:/children`
    ...
    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(`Failed to get folder contents: ${response.status} ${response.statusText} - ${errorData.error?.message || "Unknown error"}`)
    }
  ...
  /**
   * Uploads a file to a specific folder in a SharePoint site.
   * @param siteUrl The full URL of the SharePoint site.
   * @param folderPath The relative path to the target folder.
   * @param file The File object to upload.
   * @returns A promise that resolves with the uploaded file's metadata.
   */
  static async uploadFile(siteUrl: string, folderPath: string, file: File): Promise<any> {
    const accessToken = await this.getAccessToken()
    const encodedSiteUrl = encodeURIComponent(siteUrl)
    const encodedFolderPath = encodeURIComponent(folderPath)
    ...
    // Graph API path for uploading: /sites/{site-id}/drive/root:/{path}/{filename}:/content
    const url = `${graphConfig.graphSitesEndpoint}/${siteId}/drive/root:/${encodedFolderPath}/${encodedFileName}:/content`

    const response = await fetch(url, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": file.type || "application/octet-stream", // Set content type based on file
      },
      body: file,
    })
